...
namespace MyCensus.Services
{
    public class iOSNavigationService : NavigationService
    {
        ...
        {
            if (_mappings.ContainsKey(typeof(MainPageViewModel)))
            {
                _mappings[typeof(MainPageViewModel)] = typeof(iOSMainPage);
            }
            else
            {
                _mappings.Add(typeof(MainPageViewModel), typeof(iOSMainPage));
            }
        }
