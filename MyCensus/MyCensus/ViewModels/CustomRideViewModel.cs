...
{
    public class CustomRideViewModel : ViewModelBase, IActiveAware
    {
        ...
        private async void MakeBookingAsync(object obj)
        {
            ...
            try
            {
                ...
                await _navigationService.NavigateToAsync<BookingViewModel>(navParameter);
                //await _navigationService.RemoveLastFromBackStackAsync();
            }
            catch (NoAvailableBikesException)
            {
                await _dialogService.ShowAlertAsync("We are sorry, there are no bikes in origin station", "No bike available", "Ok");
            }
            catch (Exception ex) when (ex is WebException || ex is HttpRequestException)
            {
                await _dialogService.ShowAlertAsync("Communication error", "Error", "Ok");
