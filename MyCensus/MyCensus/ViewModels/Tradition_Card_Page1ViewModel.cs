...
{
    public class Tradition_Card_Page1ViewModel : ViewModelBase, IActiveAware
    {
        ...
        /// 下一步
        /// </summary>
        public ICommand NextWFCmd => new Command<string>(async (id) => { await NextWF(id); });
        private async Task NextWF(string id)
        {
            try
            {
                var valid = false;
                ...
                else
                {
                    ...
                    {
                        EndPointTelphone_Tip = "电话格式有误,如:029888888,091688888,13002900000";
                        valid = true;
                    }
                }
                ...
                if (MyCensus.Utils.MathHelper.IsNotDouble(Tradition.Latitude.ToString()) || MyCensus.Utils.MathHelper.IsNotDouble(Tradition.Longitude.ToString()))
                {
                    _dialogService.LongAlert("获取不到终端坐标,请重新尝试定位!");
                    return;
                }

                Tradition.BaseInfo = BaseInfo;
                Tradition.DoorheadPhotos.Add(new DoorheadPhoto() { StoragePath = ThumbnailPhotoUrl });
                var parameter = new NavigationParameters() {
                                { "data", Tradition },
                                { "action",Action}
                            };

                await _navigationService.NavigateAsync("Tradition_Card_Page2", parameter);
            }
            catch (Exception ex)
            {
        ...
        /// 基本信息绑定
        /// </summary>
        private TraditionBaseInfo _baseInfo;
        public TraditionBaseInfo BaseInfo
        {
            get
            {
                return _baseInfo;
        ...
        private async Task AutoPosition(string pid)
        {
            ...
            using (UserDialogs.Instance.Loading("位置获取中..."))
            {
                ...
                Device.BeginInvokeOnMainThread(() =>
                {
                    ...
                    Tradition.Latitude = GlobalSettings.CurrtntCoordinate != null ? GlobalSettings.CurrtntCoordinate.Latitude : 0;
                    Tradition.Longitude = GlobalSettings.CurrtntCoordinate != null ? GlobalSettings.CurrtntCoordinate.Longitude : 0;
                    Tradition.Location = GlobalSettings.CurrentAddComp != null ? GlobalSettings.CurrentAddComp.City : "";
